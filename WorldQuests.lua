...
local MAP_ZONES = {
	...
	[CONSTANTS.EXPANSIONS.SHADOWLANDS] = {
		...
		[1961] = { id = 1961, name = GetMapInfo(1961).name, quests = {}, buttons = {}, }, -- Korthia 9.0
...
BWQ.buttonShadowlands:SetBackdrop({
    bgFile = "Interface\\ChatFrame\\ChatFrameBackground",
    tile = false,
    tileSize = 0,
    edgeSize = 2,
...
BWQ.buttonSettings:SetScript("OnClick", function(self)
...
function BWQ:WorldQuestsUnlocked()
    if not hasUnlockedWorldQuests then
        hasUnlockedWorldQuests = 
				...
				or (expansion == CONSTANTS.EXPANSIONS.LEGION and UnitLevel("player") >= 45 and (IsQuestFlaggedCompleted(43341) or IsQuestFlaggedCompleted(45727))) -- broken isles
    end

    if not hasUnlockedWorldQuests then
        if not BWQ.errorFS then
            CreateErrorFS()
        end
        ...
        elseif expansion == CONSTANTS.EXPANSIONS.BFA then
            ...
            quest = isHorde and "|cffffff00|Hquest:57559:-1|h[Uniting Zandalar]|h|r" or
                        "|cffffff00|Hquest:51918:-1|h[Uniting Kul Tiras]|h|r"
        else -- legion
            level = "45"
...
local FormatTimeLeftString = function(timeLeft)
    ...
    if timeLeft <= 120 then
        timeLeftStr = string.format("|cffD96932%s|r", timeLeftStr)
    elseif timeLeft <= 240 then
        timeLeftStr = string.format("|cffDBA43B%s|r", timeLeftStr)
    elseif timeLeft <= 480 then
        timeLeftStr = string.format("|cffE6D253%s|r", timeLeftStr)
    elseif timeLeft <= 960 then
        timeLeftStr = string.format("|cffE6DA8E%s|r", timeLeftStr)
    end
    return timeLeftStr
end
...
local Row_OnClick = function(row)
    if IsShiftKeyDown() then
        if (GetQuestWatchType(row.quest.questId) == Enum.QuestWatchType.Manual or GetSuperTrackedQuestID() ==
            row.quest.questId) then
        ...
        if WorldMapFrame:IsShown() then
            ...
            if not row.quest.x or not row.quest.y then
                BWQ:QueryZoneQuestCoordinates(row.mapId)
            end
            if row.quest.x and row.quest.y then
                ...
                BWQ.mapTextures:SetPoint("CENTER", WorldMapFrame:GetCanvas(), "TOPLEFT", x,
                    y + 25 + (scale < 0.5 and 50 or 0))
        ...
        if TomTom and C("enableTomTomWaypointsOnClick") then
            ...
            if row.quest.x and row.quest.y then
                if currentTomTomWaypoint then
                    TomTom:RemoveWaypoint(currentTomTomWaypoint)
                end
                currentTomTomWaypoint = TomTom:AddWaypoint(row.mapId, row.quest.x, row.quest.y, {
...
local RetrieveWorldQuests = function(mapId)
    ...
    if questList then
                ...
                if questTagInfo and questTagInfo.worldQuestType then
                    ...
                    if GetNumQuestLogRewards(quest.questId) > 0 then
                        ...
                        if itemName then
                            ...
                            if classId == 7 then
                                quest.sort = quest.sort > CONSTANTS.SORT_ORDER.PROFESSION and quest.sort or
                                                 CONSTANTS.SORT_ORDER.PROFESSION
                            ...
                            elseif equipSlot ~= "" or itemId == 163857 --[[ Azerite Armor Cache ]] then
                                quest.sort = quest.sort > CONSTANTS.SORT_ORDER.EQUIP and quest.sort or
                                                 CONSTANTS.SORT_ORDER.EQUIP
                            ...
                            elseif itemId == 137642 then
                                quest.sort = quest.sort > CONSTANTS.SORT_ORDER.ITEM and quest.sort or
                                                 CONSTANTS.SORT_ORDER.ITEM
                    ...
                    if money > 20000 then -- >2g, hides these silly low gold extra rewards
                        ...
                        quest.sort = quest.sort > CONSTANTS.SORT_ORDER.MONEY and quest.sort or
                                         CONSTANTS.SORT_ORDER.MONEY
                    ...
                    if honor > 0 then
                        ...
                        quest.sort = quest.sort > CONSTANTS.SORT_ORDER.HONOR and quest.sort or
                                         CONSTANTS.SORT_ORDER.HONOR
                    ...
                    for i = 1, numQuestCurrencies do
                        ...
                        if name then
                            ...
                            if currencyId == 1553 then
                                quest.sort = quest.sort > CONSTANTS.SORT_ORDER.ARTIFACTPOWER and quest.sort or
                                                 CONSTANTS.SORT_ORDER.ARTIFACTPOWER
                            else
                                quest.sort = quest.sort > CONSTANTS.SORT_ORDER.RESOURCES and quest.sort or
                                                 CONSTANTS.SORT_ORDER.RESOURCES
                            end
                    ...
                    if quest.worldQuestType == CONSTANTS.WORLD_QUEST_TYPES.PETBATTLE then
                        ...
                        quest.isMissingAchievementCriteria =
                            BWQ:IsQuestAchievementCriteriaMissing(CONSTANTS.ACHIEVEMENT_IDS.PET_BATTLE_WQ[expansion],
                                quest.questId)
                    ...
                    if not quest.hide then
                        ...
                        if rewardType then
                            for _, rtype in next, rewardType do
                                ...
                                if rtype == CONSTANTS.REWARD_TYPES.WAKENING_ESSENCES and
                                    quest.reward.wakeningEssencesAmount then
                                    BWQ.totalWakeningEssences = BWQ.totalWakeningEssences +
                                                                    quest.reward.wakeningEssencesAmount
                                end
                                ...
                                if rtype == CONSTANTS.REWARD_TYPES.LEGIONFALL_SUPPLIES and
                                    ...
                                    BWQ.totalLegionfallSupplies = BWQ.totalLegionfallSupplies +
                                ...
                                if rtype == CONSTANTS.REWARD_TYPES.PRISMATIC_MANAPEARL then
                                    BWQ.totalPrismaticManapearl = BWQ.totalPrismaticManapearl +
                                                                      quest.reward.prismaticManapearlAmount
                                end
                            end
                        end
                        if questType then
        ...
        if C("sortByTimeRemaining") then
            table.sort(MAP_ZONES[expansion][mapId].questsSort, function(a, b)
                return MAP_ZONES[expansion][mapId].quests[a].timeLeft < MAP_ZONES[expansion][mapId].quests[b].timeLeft
            end)
        else -- reward type
            table.sort(MAP_ZONES[expansion][mapId].questsSort, function(a, b)
                return MAP_ZONES[expansion][mapId].quests[a].sort > MAP_ZONES[expansion][mapId].quests[b].sort
            end)
        end

        if DEBUG then
            print(string.format("[BWQ] number of quests after filtering for map %s is %s", mapId, numQuests))
        end
...
function BWQ:UpdateBountyData()
    ...
    for bountyIndex, bounty in ipairs(bounties) do
        ...
        if not BWQ.bountyCache[bountyIndex] then
            ...
            bountyCacheItem.text = BWQ.bountyDisplay:CreateFontString("BWQ_BountyDisplayText" .. bountyIndex, "OVERLAY",
        ...
        if bounty.icon and title then
            ...
            bountyCacheItem.text:SetText(string.format("|cff%s%s\n %s/%s      |r%s",
                numFulfilled == numRequired and "49d65e" or "fafafa", title, numFulfilled or 0, numRequired or 0,
            ...
            bountyCacheItem.button:SetScript("OnEnter", function(self)
                BWQ:ShowBountyTooltip(self, bounty.questID)
            end)
            bountyCacheItem.button:SetScript("OnLeave", function(self)
                GameTooltip:Hide()
            end)
...
function BWQ:ShowBountyTooltip(button, questId)
    ...
    if title then
        ...
        button.UpdateTooltip = function(self)
            BWQ:ShowBountyTooltip(button, questId)
        end
    end
end
...
function BWQ:OnFactionUpdate(msg)
    if C("hideFactionParagonBars") then
        return
    end
    ...
    for _, factionId in next, paragonFactions.order do
        if IsFactionParagon(factionId) then
            ...
            if not BWQ.factionFramePool.rows[rowIndex] then
                ...
                row = BWQ.factionFramePool.rows[rowIndex]
            end

            if not BWQ.factionFramePool.bars[i] then
                factionFrame = {}
                factionFrame.name = row:CreateFontString("BWQ_FactionDisplayName" .. i, "OVERLAY",
                    "SystemFont_Shadow_Med1")
            ...
            if (rowIndex == 0) then
                row:SetPoint("TOP", BWQ.factionDisplay, "TOP", 0, 0)
            else
                row:SetPoint("TOP", BWQ.factionFramePool.rows[rowIndex - 1], "BOTTOM", 0, -5)
            end
            row:Show()
            ...
            if current and threshold then
                progress = (current % threshold) / threshold * 50
            end
            if hasRewardPending then
                factionFrame.bar:SetBackdropColor(0, 0.8, 0.1)
            else
                factionFrame.bar:SetBackdropColor(0.1, 0.55, 0.1, 0.4)
            end
            if progress == 0 then
                factionFrame.bar:Hide()
            else
...
function BWQ:RenderRows()
    ...
    if showDownwards then
        availableHeight = screenHeight - (screenHeight - blockYPos) - 30
    else
        availableHeight = screenHeight - blockYPos - 30
    end
    ...
    for _, mapId in next, MAP_ZONES_SORT[expansion] do
        ...
        if MAP_ZONES[expansion][mapId].numQuests == 0 or rowIndex < sliderval or rowIndex > sliderval + maxEntries then
            ...
            MAP_ZONES[expansion][mapId].zoneSep.fs:SetPoint("TOP", BWQ, "TOP", 15 + (totalWidth / -2) +
                (MAP_ZONES[expansion][mapId].zoneSep.fs:GetStringWidth() / 2),
                offsetTop + ROW_HEIGHT * rowInViewIndex - 2)
            MAP_ZONES[expansion][mapId].zoneSep.texture:Show()
            MAP_ZONES[expansion][mapId].zoneSep.texture:SetPoint("TOP", BWQ, "TOP", 5,
                offsetTop + ROW_HEIGHT * rowInViewIndex - 3)
...
function BWQ:UpdateBlock()
    ...
    for mapId in next, MAP_ZONES[expansion] do
        ...
        if not MAP_ZONES[expansion][mapId].zoneSep then
            ...
            if faction then
                local factionIcon = faction == CONSTANTS.FACTIONS.HORDE and
                                        "Interface\\Icons\\inv_misc_tournaments_banner_orc" or
            ...
            for _, questId in next, MAP_ZONES[expansion][mapId].questsSort do
                ...
                if button.quest.reward.itemName then
                    local itemText = string.format("%s[%s%s]|r",
                        ITEM_QUALITY_COLORS[button.quest.reward.itemQuality].hex, button.quest.reward.realItemLevel and
                            (button.quest.reward.realItemLevel .. " ") or "", button.quest.reward.itemName)

                    rewardText = string.format("|T%s$s:14:14|t %s%s", button.quest.reward.itemTexture, button.quest
                        .reward.itemQuantity > 1 and button.quest.reward.itemQuantity .. "x " or "", itemText)
                ...
                if button.quest.reward.honor and button.quest.reward.honor > 0 then
                    rewardText = string.format("%1$s%2$s|T%3$s:14:14|t %4$d %5$s", rewardText,
                        rewardText ~= "" and "   " or "", -- insert some space between rewards
                        "Interface\\Icons\\Achievement_LegionPVPTier4", button.quest.reward.honor, HONOR)
                end
                if button.quest.reward.money and button.quest.reward.money > 0 then
                    ...
                    rewardText =
                        string.format("%s%s%s", rewardText, rewardText ~= "" and "   " or "", -- insert some space between rewards
                            moneyText)
                end
                ...
                button.titleFS:SetText(string.format("%s%s%s|r", button.quest.isNew and "|cffe5cc80NEW|r  " or "",
                    button.quest.isMissingAchievementCriteria and "|cff1EFF00" or
                        WORLD_QUEST_QUALITY_COLORS[button.quest.quality].hex, button.quest.title))
                -- local titleWidth = button.titleFS:GetStringWidth()
                -- if titleWidth > titleMaxWidth then titleMaxWidth = titleWidth end

                if GetQuestWatchType(button.quest.questId) == Enum.QuestWatchType.Manual or GetSuperTrackedQuestID() ==
                    button.quest.questId then
                    button.track:Show()
                else
                    button.track:Hide()
                end
                ...
                if bountyWidth > bountyMaxWidth then
                    bountyMaxWidth = bountyWidth
                end
                ...
                if not C("hideFactionColumn") then
                    ...
                    if factionWidth > factionMaxWidth then
                        factionMaxWidth = factionWidth
                    end
                else
                ...
                if rewardWidth > rewardMaxWidth then
                    rewardMaxWidth = rewardWidth
                end
    ...
    if C("showTotalsInBrokerText") then
        local brokerString = ""
        if C("brokerShowAP") and BWQ.totalArtifactPower > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\inv_smallazeriteshard:16:16|t %s  ", brokerString,
                AbbreviateNumber(BWQ.totalArtifactPower))
        end
        if C("brokerShowServiceMedals") and BWQ.totalServiceMedals > 0 then
            brokerString = string.format("%s|T%s:16:16|t %s  ", brokerString, isHorde and
                "Interface\\Icons\\ui_horde_honorboundmedal" or "Interface\\Icons\\ui_alliance_7legionmedal",
                BWQ.totalServiceMedals)
        end
        if C("brokerShowWakeningEssences") and BWQ.totalWakeningEssences > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\achievement_dungeon_ulduar80_25man:16:16|t %s  ",
                brokerString, BWQ.totalWakeningEssences)
        end
        if C("brokerShowWarResources") and BWQ.totalWarResources > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\inv__faction_warresources:16:16|t %d  ", brokerString,
                BWQ.totalWarResources)
        end
        if C("brokerShowPrismaticManapearl") and BWQ.totalPrismaticManapearl > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\Inv_misc_enchantedpearlf:16:16|t %d  ", brokerString,
                BWQ.totalPrismaticManapearl)
        end
        if C("brokerShowResources") and BWQ.totalResources > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\inv_orderhall_orderresources:16:16|t %d  ",
                brokerString, BWQ.totalResources)
        end
        if C("brokerShowLegionfallSupplies") and BWQ.totalLegionfallSupplies > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\inv_misc_summonable_boss_token:16:16|t %d  ",
                brokerString, BWQ.totalLegionfallSupplies)
        end
        if C("brokerShowHonor") and BWQ.totalHonor > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\Achievement_LegionPVPTier4:16:16|t %d  ", brokerString,
                BWQ.totalHonor)
        end
        if C("brokerShowGold") and BWQ.totalGold > 0 then
            brokerString = string.format("%s|TInterface\\GossipFrame\\auctioneerGossipIcon:16:16|t %d  ", brokerString,
                math.floor(BWQ.totalGold / 10000))
        end
        if C("brokerShowGear") and BWQ.totalGear > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\Inv_chest_plate_legionendgame_c_01:16:16|t %d  ",
                brokerString, BWQ.totalGear)
        end
        if C("brokerShowMarkOfHonor") and BWQ.totalMarkOfHonor > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\ability_pvp_gladiatormedallion:16:16|t %d  ",
                brokerString, BWQ.totalMarkOfHonor)
        end
        if C("brokerShowHerbalism") and BWQ.totalHerbalism > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\Trade_Herbalism:16:16|t %d  ", brokerString,
                BWQ.totalHerbalism)
        end
        if C("brokerShowMining") and BWQ.totalMining > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\Trade_Mining:16:16|t %d  ", brokerString,
                BWQ.totalMining)
        end
        if C("brokerShowFishing") and BWQ.totalFishing > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\Trade_Fishing:16:16|t %d  ", brokerString,
                BWQ.totalFishing)
        end
        if C("brokerShowSkinning") and BWQ.totalSkinning > 0 then
            brokerString = string.format("%s|TInterface\\Icons\\inv_misc_pelt_wolf_01:16:16|t %d  ", brokerString,
                BWQ.totalSkinning)
        end
        if C("brokerShowBloodOfSargeras") and BWQ.totalBloodOfSargeras > 0 then
            brokerString = string.format("%s|T1417744:16:16|t %d", brokerString, BWQ.totalBloodOfSargeras)
        end
...
function BWQ:SetupConfigMenu()
    local options = {{
        text = "Attach list frame to world map",
        check = "attachToWorldMap"
    }, {
        text = "Show list frame on click instead of mouse-over",
        check = "showOnClick"
    }, {
        text = "Use per-character settings",
        check = "usePerCharacterSettings"
    }, {
        text = ""
    }, {
        text = "Always show |cffa335eeepic|r world quests (e.g. world bosses)",
        check = "alwaysShowEpicQuests"
    }, {
        text = "Only show world quests with |cff0070ddrare|r or above quality",
        check = "onlyShowRareOrAbove"
    }, {
        text = "Don't filter quests for active bounties",
        check = "alwaysShowBountyQuests"
    }, {
        text = "Show total counts in broker text",
        check = "showTotalsInBrokerText",
        submenu = {{
            text = ("|T%1$s:16:16|t  Artifact Power"):format("Interface\\Icons\\inv_smallazeriteshard"),
            check = "brokerShowAP"
        }, {
            text = ("|T%1$s:16:16|t  Service Medals"):format(
                isHorde and "Interface\\Icons\\ui_horde_honorboundmedal" or "Interface\\Icons\\ui_alliance_7legionmedal"),
            check = "brokerShowServiceMedals"
        }, {
            text = ("|T%1$s:16:16|t  Wakening Essences"):format("Interface\\Icons\\achievement_dungeon_ulduar80_25man"),
            check = "brokerShowWakeningEssences"
        }, {
            text = ("|T%1$s:16:16|t  Prismatic Manapearls"):format("Interface\\Icons\\Inv_misc_enchantedpearlf"),
            check = "brokerShowPrismaticManapearl"
        }, {
            text = ("|T%1$s:16:16|t  War Resources"):format("Interface\\Icons\\inv__faction_warresources"),
            check = "brokerShowWarResources"
        }, {
            text = ("|T%1$s:16:16|t  Order Hall Resources"):format("Interface\\Icons\\inv_orderhall_orderresources"),
            check = "brokerShowResources"
        }, {
            text = ("|T%1$s:16:16|t  Legionfall War Supplies"):format("Interface\\Icons\\inv_misc_summonable_boss_token"),
            check = "brokerShowLegionfallSupplies"
        }, {
            text = ("|T%1$s:16:16|t  Honor"):format("Interface\\Icons\\Achievement_LegionPVPTier4"),
            check = "brokerShowHonor"
        }, {
            text = ("|T%1$s:16:16|t  Gold"):format("Interface\\GossipFrame\\auctioneerGossipIcon"),
            check = "brokerShowGold"
        }, {
            text = ("|T%1$s:16:16|t  Gear"):format("Interface\\Icons\\Inv_chest_plate_legionendgame_c_01"),
            check = "brokerShowGear"
        }, {
            text = ("|T%1$s:16:16|t  Mark Of Honor"):format("Interface\\Icons\\ability_pvp_gladiatormedallion"),
            check = "brokerShowMarkOfHonor"
        }, {
            text = ("|T%1$s:16:16|t  Herbalism Quests"):format("Interface\\Icons\\Trade_Herbalism"),
            check = "brokerShowHerbalism"
        }, {
            text = ("|T%1$s:16:16|t  Mining Quests"):format("Interface\\Icons\\Trade_Mining"),
            check = "brokerShowMining"
        }, {
            text = ("|T%1$s:16:16|t  Fishing Quests"):format("Interface\\Icons\\Trade_Fishing"),
            check = "brokerShowFishing"
        }, {
            text = ("|T%1$s:16:16|t  Skinning Quests"):format("Interface\\Icons\\inv_misc_pelt_wolf_01"),
            check = "brokerShowSkinning"
        }, {
            text = ("|T%s$s:16:16|t  Blood of Sargeras"):format("1417744"),
            check = "brokerShowBloodOfSargeras"
        }}
    }, {
        text = "Sort list by time remaining instead of reward type",
        check = "sortByTimeRemaining"
    }, {
        text = ""
    }, {
        text = "Filter by reward...",
        isTitle = true
    }, {
        text = ("|T%1$s:16:16|t  Azerite"):format("Interface\\Icons\\inv_smallazeriteshard"),
        check = "showArtifactPower"
    }, {
        text = ("|T%1$s:16:16|t  Prismatic Manapearl"):format("Interface\\Icons\\Inv_misc_enchantedpearlf"),
        check = "showPrismaticManapearl"
    }, {
        text = ("|T%1$s:16:16|t  Items"):format("Interface\\Minimap\\Tracking\\Banker"),
        check = "showItems",
        submenu = {{
            text = ("|T%1$s:16:16|t  Gear"):format("Interface\\Icons\\Inv_chest_plate_legionendgame_c_01"),
            check = "showGear"
        }, {
            text = ("|T%s$s:16:16|t  Crafting Materials"):format("1417744"),
            check = "showCraftingMaterials"
        }, {
            text = ("|T%1$s:16:16|t  Mark Of Honor"):format("Interface\\Icons\\ability_pvp_gladiatormedallion"),
            check = "showMarkOfHonor"
        }, {
            text = "Other",
            check = "showOtherItems"
        }}
    }, {
        ...
        text = ("|T%1$s:16:16|t  Low gold reward"):format("Interface\\GossipFrame\\auctioneerGossipIcon"),
        check = "showLowGold"
    }, {
        text = ("|T%1$s:16:16|t  High gold reward"):format("Interface\\GossipFrame\\auctioneerGossipIcon"),
        check = "showHighGold"
    }, {
        text = ("|T%1$s:16:16|t  War Resources"):format("Interface\\Icons\\inv__faction_warresources"),
        check = "showWarResources"
    }, {
        text = "       Legion",
        submenu = {{
            text = ("|T%1$s:16:16|t  Order Hall Resources"):format("Interface\\Icons\\inv_orderhall_orderresources"),
            ...
            text = ("|T%1$s:16:16|t  Legionfall War Supplies"):format("Interface\\Icons\\inv_misc_summonable_boss_token"),
            check = "showLegionfallSupplies"
        }, {
            text = ("|T%1$s:16:16|t  Nethershard"):format("Interface\\Icons\\inv_datacrystal01"),
            check = "showNethershards"
        }, {
            text = ("|T%1$s:16:16|t  Veiled Argunite"):format("Interface\\Icons\\oshugun_crystalfragments"),
            check = "showArgunite"
        }, {
            text = ("|T%1$s:16:16|t  Wakening Essences"):format("Interface\\Icons\\achievement_dungeon_ulduar80_25man"),
            check = "showWakeningEssences"
        }}
    }, {
        text = ""
    }, {
        ...
        text = "Profession Quests",
        check = "showProfession",
        submenu = {{
            text = "Alchemy",
            check = "showProfessionAlchemy"
        }, {
            text = "Blacksmithing",
            check = "showProfessionBlacksmithing"
        }, {
            text = "Inscription",
            check = "showProfessionInscription"
        }, {
            text = "Jewelcrafting",
            check = "showProfessionJewelcrafting"
        }, {
            text = "Leatherworking",
            check = "showProfessionLeatherworking"
        }, {
            text = "Tailoring",
            check = "showProfessionTailoring"
        }, {
            text = "Enchanting",
            check = "showProfessionEnchanting"
        }, {
            text = "Engineering",
            check = "showProfessionEngineering"
        }, {
            text = ""
        }, {
            text = "Herbalism",
            check = "showProfessionHerbalism"
        }, {
            text = "Mining",
            check = "showProfessionMining"
        }, {
            text = "Skinning",
            check = "showProfessionSkinning"
        }, {
            text = ""
        }, {
            text = "Cooking",
            check = "showProfessionCooking"
        }, {
            text = "Archaeology",
            check = "showProfessionArchaeology"
        }, {
            text = "Fishing",
            check = "showProfessionFishing"
        }}
    }, {
        text = "Dungeon Quests",
        check = "showDungeon"
        ...
        text = "Pet Battle Quests",
        check = "showPetBattle",
        submenu = {{
            text = "Hide Pet Battle Quests even when active bounty",
            check = "hidePetBattleBountyQuests"
        }, {
            text = "Always show quests for \"Family Familiar\" achievement",
            check = "alwaysShowPetBattleFamilyFamiliar"
        }}
    }, {
        text = ""
    }, {
        text = "Hide faction column",
        check = "hideFactionColumn"
    }, {
        text = "Hide faction paragon bars",
        check = "hideFactionParagonBars"
    }, {
        ...
        submenu = {{
            text = "The Avowed",
            check = "alwaysShowAvowed"
        }, {
            text = "The Wild Hunt",
            check = "alwaysShowWildHunt"
        }, {
            text = "Court of Harvesters",
            check = "alwaysShowCourtofHarvesters"
        }, {
            text = "The Undying Army",
            check = "alwaysShowUndyingArmy"
        }, {
            text = "The Ascended",
            check = "alwaysShowAscended"
        }}
    }, {
        text = "       Battle for Azeroth",
        submenu = {{
            text = "Rustbolt Resistance",
            check = "alwaysShowRustboltResistance"
        }, {
            text = "Tortollan Seekers",
            check = "alwaysShowTortollanSeekers"
        }, {
            text = "Champions of Azeroth",
            check = "alwaysShowChampionsOfAzeroth"
        }, {
            text = ("|T%1$s:16:16|t  7th Legion"):format("Interface\\Icons\\inv_misc_tournaments_banner_human"),
            check = "alwaysShow7thLegion"
        }, {
            text = ("|T%1$s:16:16|t  Storm's Wake"):format("Interface\\Icons\\inv_misc_tournaments_banner_human"),
            check = "alwaysShowStormsWake"
        }, {
            text = ("|T%1$s:16:16|t  Order of Embers"):format("Interface\\Icons\\inv_misc_tournaments_banner_human"),
            check = "alwaysShowOrderOfEmbers"
        }, {
            text = ("|T%1$s:16:16|t  Proudmoore Admiralty"):format("Interface\\Icons\\inv_misc_tournaments_banner_human"),
            check = "alwaysShowProudmooreAdmiralty"
        }, {
            text = ("|T%1$s:16:16|t  Waveblade Ankoan"):format("Interface\\Icons\\inv_misc_tournaments_banner_human"),
            check = "alwaysShowWavebladeAnkoan"
        }, {
            text = ("|T%1$s:16:16|t  The Honorbound"):format("Interface\\Icons\\inv_misc_tournaments_banner_orc"),
            check = "alwaysShowTheHonorbound"
        }, {
            text = ("|T%1$s:16:16|t  Zandalari Empire"):format("Interface\\Icons\\inv_misc_tournaments_banner_orc"),
            check = "alwaysShowZandalariEmpire"
        }, {
            text = ("|T%1$s:16:16|t  Talanji's Expedition"):format("Interface\\Icons\\inv_misc_tournaments_banner_orc"),
            check = "alwaysShowTalanjisExpedition"
        }, {
            text = ("|T%1$s:16:16|t  Voldunai"):format("Interface\\Icons\\inv_misc_tournaments_banner_orc"),
            check = "alwaysShowVoldunai"
        }, {
            text = ("|T%1$s:16:16|t  The Unshackled"):format("Interface\\Icons\\inv_misc_tournaments_banner_orc"),
            check = "alwaysShowTheUnshackled"
        }}
    }, {
        text = "       Legion",
        submenu = {{
            text = "Court of Farondis",
            check = "alwaysShowCourtOfFarondis"
        }, {
            text = "Dreamweavers",
            check = "alwaysShowDreamweavers"
        }, {
            text = "Highmountain Tribe",
            check = "alwaysShowHighmountainTribe"
        }, {
            text = "The Nightfallen",
            check = "alwaysShowNightfallen"
        }, {
            text = "The Wardens",
            check = "alwaysShowWardens"
        }, {
            text = "Valarjar",
            check = "alwaysShowValarjar"
        }, {
            text = "Armies of Legionfall",
            check = "alwaysShowArmiesOfLegionfall"
        }, {
            text = "Army of the Light",
            check = "alwaysShowArmyOfTheLight"
        }, {
            text = "Argussian Reach",
            check = "alwaysShowArgussianReach"
        }}
    }}
    if TomTom then
        ...
        table.insert(options, {
            text = "Add TomTom waypoint on row click",
            check = "enableTomTomWaypointsOnClick"
        })
    end
    ...
    configMenu.initialize = function(self, level)
        ...
        for i, v in ipairs(opt) do
            ...
            if v.check then
                if (v.check == "usePerCharacterSettings") then
                    info.checked = BWQcfg[v.check]
                elseif v.radio then
                    info.checked = C(v.check) == v.val
                else
                    info.checked = C(v.check)
                end
...
local SetFlightMapPins = function(self)
    for pin, active in self:GetMap():EnumeratePinsByTemplate("WorldQuestPinTemplate") do
        if C_SuperTrack.GetSuperTrackedQuestID() == pin.questID then
            ...
            if FlightMapFrame.ScrollContainer:IsZoomedOut() then
                pin:Hide()
            end
        end
    end
...
BWQ:SetScript("OnEvent", function(self, event, arg1)
    ...
    elseif event == "ADDON_LOADED" then
        if arg1 == "Broker_WorldQuests" then
            ...
            for i, v in next, defaultConfig do
                ...
                if BWQcfgPerCharacter[i] == nil then
                    BWQcfgPerCharacter[i] = v
                end
            end
    ...
    elseif event == "QUEST_ACCEPTED" then
        if TomTom and currentTomTomWaypoint and (GetTitleForLogIndex(arg1) == currentTomTomWaypoint.title) then
            TomTom:RemoveWaypoint(currentTomTomWaypoint)
        end
    elseif event == "PLAYER_LOGOUT" then
        if TomTom and currentTomTomWaypoint then
            TomTom:RemoveWaypoint(currentTomTomWaypoint)
        end
    end
end)
